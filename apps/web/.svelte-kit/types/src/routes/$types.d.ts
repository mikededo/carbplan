...
type LayoutRouteId = RouteId | "/(marketing)" | "/(private)/(app)/(admin)/supplements" | "/(private)/(app)/dashboard" | "/(private)/(app)/plans" | "/(private)/(app)/plans/[id]" | "/(private)/(app)/plans/new" | "/(private)/(app)/settings" | "/(private)/onboarding" | "/(public)/auth/log-in" | "/(public)/auth/sign-up" | null
